...
public class VirusEnterOnScreen2 : MonoBehaviour, IEnterOnScreen{
    ...
    private float initialSecondPhaseTime;
    ...
    public void EnterOnScreen(){
        if(CanEnterOnScreen.CanEnterOnScreen()){
            ...
            if(directionAnimation * PontoDaCurva() < directionAnimation * animationTime){
                EnterOnScreenPhaseOne = false;
                if(secondPhaseTime > 0){
                    if(initialSecondPhaseTime < secondPhaseTime + 1f)
    ...
    private void CalculateOrigin(){
        if(!instantiateOrigin){
            ...
            initialSecondPhaseTime = secondPhaseTime;
            if(b >= 0)
